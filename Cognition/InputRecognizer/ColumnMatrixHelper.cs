...
class ColumnMatrixHelper
{
    ...
    public static MatrixJson MatrixToJson(List<string[]> matrixData, string fileName)
    {
        ...
        for (int i = 0; i < matrixData.Count; i++)
        {
            int latestIndex = matrixData[i].Length - 1;
            connections.Add(new ConnectionsJson { from = Int32.Parse(matrixData[i][0]), to = Int32.Parse(matrixData[i][1]), value = Helper.ConvertToDouble(matrixData[i][latestIndex]) });
        }
